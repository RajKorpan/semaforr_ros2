...
| Passage grid | Dissertation-faithful data semantics; HLE inference remains a temporary approximation | Free/obstructed/passage-number labels updated locally during HLE | Sparse typed free, obstructed, and passage cells; passage cells carry stable candidate IDs and separate evidence counts | No for cell semantics | Downstream code can distinguish obstruction and passage identity; HLE cue and termination behavior still differs | Algorithm-level for cell semantics only | Complete the published cue and pursuit rules |
...
| HLE | Temporary approximation | Left/right fixed ray bundles, length/width and large-room tests, segment-overlap cue similarity, numbered passage grid, rich pursuit termination | Finds contiguous clear bundles, hashes cue endpoints, pursues a short displacement, and records typed numbered passages plus free/obstructed scan evidence | No | Exploration coverage, path, passage discovery, and time use differ substantially even though the passage-grid data contract is restored | No | Implement the published cue and pursuit state machine behind compatibility mode |
