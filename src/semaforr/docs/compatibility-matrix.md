...
| Enforcer | Functionally adapted | Tier-1 action selector with one-step lookahead; operationalizes typed steps every decision | Converts hierarchical or recovery-plan waypoints into local Tier-1 actions before reactive planners | Safety/action typing adaptation | Plan-following retains Tier-1 priority with typed action magnitudes | Algorithm-level, subject to typed action repertoire | Retain ordering and recovery-plan tests |
...
| HLE explanation | Functionally adapted diagnostics; natural-language Why remains incomplete | Generic exploration action/confidence/alternative templates | Exposes state, lifecycle event, persistent candidate geometry/confidence, explicit rejection or termination reason, passage revision, and exact replay trace; no question-driven prose template yet | Structured diagnostics are intentional | Decisions are auditable and replayable, but Chapter 5 wording is not reproduced | No for Chapter 5 prose; yes for structured evidence | Build question-driven templates over the HLE trace |
