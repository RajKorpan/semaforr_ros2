...
|---|---|---|---|---|---|---|---|
| Decision path/history | Temporary approximation | Record the completed decision point, action, pose, and view every decision | Records the observation and selected action before execution outcome is returned | No | Failed and partial actions are indistinguishable to learners | No | Add post-execution learning episodes with actual outcome and achieved pose |
