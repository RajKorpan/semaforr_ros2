  ...
  const rclcpp::Time& received_at)
{
  ...
  pose_ = PoseSample{
    Position(
      ...
      domain::Angle::normalize(yaw)),
    sourceStamp(message.header.stamp, received_at),
    received_at};
  pose_error_.reset();
  ++generation_;
  return true;
...
bool SensorSynchronizer::acceptScan(
  ...
  const rclcpp::Time& received_at)
{
  ...
  if (!std::isfinite(message.angle_min) ||
      ...
      !std::isfinite(message.range_max) ||
...
{
  switch (status) {
    case SensorStatus::WaitingForPose: return "waiting_for_pose";
    case SensorStatus::WaitingForScan: return "waiting_for_scan";
    case SensorStatus::PoseFrameMismatch: return "pose_frame_mismatch";
    case SensorStatus::ScanFrameMismatch: return "scan_frame_mismatch";
    case SensorStatus::InvalidPose: return "invalid_pose";
    case SensorStatus::InvalidScan: return "invalid_scan";
    case SensorStatus::PoseStale: return "pose_stale";
    case SensorStatus::ScanStale: return "scan_stale";
    case SensorStatus::Unsynchronized: return "unsynchronized";
    case SensorStatus::ClockReset: return "clock_reset";
