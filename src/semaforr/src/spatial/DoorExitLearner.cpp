...
DoorExitLearner::DoorExitLearner(
  double minimum_range_jump_m,
  double maximum_opening_width_m)
  : SpatialLearnerBase(
      SpatialRepresentation::DoorsAndExits,
      "door_exit",
...
{
  if (!std::isfinite(minimum_range_jump_m_) ||
      minimum_range_jump_m_ <= 0.0 ||
