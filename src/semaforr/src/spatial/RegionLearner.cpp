...
RegionLearner::RegionLearner(
  double cluster_radius_m,
  std::size_t minimum_observations)
  : SpatialLearnerBase(
      SpatialRepresentation::Regions,
      "region",
    ...
    minimum_observations_(minimum_observations)
{
  ...
  if (minimum_observations_ == 0U) {
    throw std::invalid_argument(
      "region minimum observations must be positive");
  }
}
...
void RegionLearner::onRebuild()
{
  ...
  publish(
    ...
    model.regions.empty() ? "no pose cluster reached the observation threshold" :
