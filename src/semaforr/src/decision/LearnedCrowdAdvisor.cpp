...
std::pair<domain::Point2D, domain::Angle>
LearnedCrowdAdvisor::expected(
  ...
  const domain::Action& action) const
{
  ...
  if (action.type() == domain::ActionType::Forward) {
    if (magnitude == 0U ||
        magnitude > configuration_.move_distances_m.size()) {
  ...
  std::span<const domain::Action> candidates) const
{
  ...
  evaluation.explanation =
    "shared visibility-normalized learned crowd field";
  ...
  if (!learned &&
      (configuration_.objective !=
        LearnedCrowdObjective::AvoidEncounterRisk || !live_people)) {
    evaluation.explanation =
  ...
  for (const auto& action : candidates) {
    ...
    switch (configuration_.objective) {
      ...
      case LearnedCrowdObjective::AvoidEncounterRisk:
        if ((!sample || sample->stale ||
            sample->cell.confidence <
              configuration_.minimum_cell_confidence) && !live_people) {
          continue;
        }
