...
domain::Point2D interpolate(
  const domain::Point2D& from,
  const domain::Point2D& to,
  double fraction)
{
  return {
...
double euclideanDistance(
  const domain::Point2D& left,
  const domain::Point2D& right)
{
  return std::hypot(left.x_m - right.x_m, left.y_m - right.y_m);
}

domain::Point2D pedestrianAt(
  const domain::PedestrianObservation& pedestrian,
  domain::SocialTimestamp observed_at,
  double seconds)
{
  const auto target = observed_at +
      ...
      if (target <= prediction.predicted_at) {
        const double span =
          std::chrono::duration<double>(
            prediction.predicted_at - previous_time).count();
  ...
  return {
    pedestrian.position.x_m + pedestrian.velocity_mps.x_m * seconds,
  ...
  : configuration_(std::move(configuration))
{
  ...
  if (configuration_.maximum_age < std::chrono::nanoseconds::zero()) {
    throw std::invalid_argument(
      "social maximum age must be non-negative");
  }
  ...
  validatePositive(
    configuration_.prediction_horizon_s, "social prediction horizon");
  validatePositive(configuration_.personal_space_m, "personal space");
  validatePositive(
    configuration_.collision_distance_m, "collision distance");
  if (configuration_.collision_distance_m >
...
double SocialNavigationAdvisor::score(
  ...
  const domain::Action& action) const
{
  ...
  if (action.type() == domain::ActionType::Forward) {
    ...
    const double travel = configuration_.move_distances_m[index - 1U];
      ...
      start.x_m + travel * std::cos(world.robot.pose.heading.radians()),
  ...
  for (const auto& pedestrian : crowd.pedestrians) {
    ...
    double minimum_separation =
      euclideanDistance(start, pedestrian.position);
    for (int sample = 1; sample <= samples; ++sample) {
      ...
      minimum_separation =
        std::min(
          minimum_separation, euclideanDistance(robot, person));
    }
    ...
    if (robot_speed > 0.0) {
      const double heading_x =
        std::cos(world.robot.pose.heading.radians());
      const double heading_y =
        std::sin(world.robot.pose.heading.radians());
      const double along =
        pedestrian.velocity_mps.x_m * heading_x +
        pedestrian.velocity_mps.y_m * heading_y;
      const double lateral = std::fabs(
        pedestrian.velocity_mps.x_m * heading_y -
        pedestrian.velocity_mps.y_m * heading_x);
      const double ahead =
        (pedestrian.position.x_m - start.x_m) * heading_x +
