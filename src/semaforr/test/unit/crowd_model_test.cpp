...
TEST(CrowdFieldLearner, UsesVisibilityAsDensityDenominator)
{
  ...
  const semaforr::domain::Pose2D robot{
    {1.5, 1.5}, semaforr::domain::Angle::zero()};
  ...
  EXPECT_DOUBLE_EQ(
    occupied->cell.directional_flow[
...
{
  semaforr::social::CrowdFieldLearner first(configuration(
    semaforr::social::CrowdEstimatorStrategy::Thompson));
  semaforr::social::CrowdFieldLearner second(configuration(
    semaforr::social::CrowdEstimatorStrategy::Thompson));
  const semaforr::domain::Pose2D robot{
    {1.5, 1.5}, semaforr::domain::Angle::zero()};
...
TEST(CrowdModel, SerializesAndRemainsUsefulWithoutLivePeople)
{
  ...
  const auto restored =
    semaforr::domain::CrowdFieldSnapshot::load(stream);
  ...
  model.clearCurrent();
  EXPECT_EQ(
    model.status(), semaforr::domain::CrowdModelStatus::LearnedOnly);
  EXPECT_GT(model.densityAt({2.2, 1.5}), 0.0);
  EXPECT_GT(
    model.flowAlignmentAt(
      {2.2, 1.5}, semaforr::domain::Angle::zero()),
    0.0);
}
...
TEST(CrowdModel, CanonicalPredictionsProduceDeterministicCollisionRisk)
{
  ...
  observation.pedestrians.front().predicted_trajectory = {
    {{4.0, 4.0}, 2s},
    {{4.5, 4.0}, 3s}};
  ...
  const double on_prediction =
    model.predictiveCollisionRiskAt({4.0, 4.0});
...
TEST(CrowdConsumers, RiskAdvisorAndPlannerShareLivePredictionRisk)
{
  ...
  const double planner_risk = planner.riskCost(400, 400, 0);
  EXPECT_DOUBLE_EQ(planner_risk, 1.0);
  EXPECT_DOUBLE_EQ(
    evaluation.scores.front().raw_score, -planner_risk);
}
...
TEST(CrowdConsumers, AdvisorAndPlannerReadTheSameLearnedCell)
{
  ...
  semaforr::domain::WorldModel world;
  world.robot.pose = {
    {1.2, 1.5}, semaforr::domain::Angle::zero()};
  world.crowd = model;
  semaforr::decision::LearnedCrowdAdvisor advisor({
    semaforr::decision::LearnedCrowdObjective::AvoidDensity,
    {1.0}, {}, 1.0, 0.0, "crowd_avoid"});
  const std::vector<semaforr::domain::Action> actions{
    semaforr::domain::Action(
      semaforr::domain::ActionType::Forward, 1U)};
  const auto evaluation = advisor.evaluate(
  ...
  PathPlanner planner(Node{}, Node{}, "density");
  planner.setCrowdModel(model);
  const double planner_cost = planner.cellCost(220, 150, 0);
  EXPECT_DOUBLE_EQ(evaluation.scores.front().raw_score, -planner_cost);
}
...
TEST(CrowdConsumers, StableRegistryConstructsAllSocialAdvisors)
{
  ...
  semaforr::decision::registerSocialAdvisorFactories(
