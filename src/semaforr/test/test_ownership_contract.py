...
def test_owners_use_values_or_unique_ptrs():
    controller = (
        SOURCE_DIR / "include" / "semaforr" / "decision" / "Controller.h"
    ).read_text(encoding="utf-8")
    agent_state = (
        SOURCE_DIR / "include" / "semaforr" / "decision" / "AgentState.h"
    ).read_text(encoding="utf-8")
    graph = (
        SOURCE_DIR / "include" / "semaforr" / "navigation" / "Graph.h"
    ).read_text(encoding="utf-8")
    node = (
        SOURCE_DIR / "src" / "ros" / "semaforr_node.cpp"
    ).read_text(encoding="utf-8")
    visualizer = (
        SOURCE_DIR / "include" / "semaforr" / "ros" / "Visualizer.h"
    ).read_text(encoding="utf-8")
